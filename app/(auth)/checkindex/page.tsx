...
const IndexPage = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (count !== 1) {
        setError(
          `${translations[locale].texts.line} ${i + 1} ${
            translations[locale].messages.checkindexErrorText
          }`
        );
        setDisabled(true);
        return;
      ...
      if (!urlMatch || !isValidUrl(urlMatch[0])) {
        setError(
          `${translations[locale].texts.line} ${i + 1} ${
            translations[locale].messages.checkindexErrorTexts
          }`
        );
        setDisabled(true);
        return;
  ...
  const filteredResults = results.filter((item) => {
    ...
    if (indexedFilter === "index")
      return item.indexed.toLowerCase() === "index";
    if (indexedFilter === "noindex")
      return item.indexed.toLowerCase() === "no index";
    return true;
  });
  ...
  return (
    <div className="pageBackground">
      <Box style={{ zIndex: 1000, position: "fixed", width: "100%" }}>
        <Header />
      </Box>
      ...
      <Grid align="center" style={{ overflow: "visible" }}>
          ...
          <Container size="xl" py="xl">
            <Box className="Boxtitle">
              <Group justify="center" gap="xl">
                ...
                <Title order={4}>
                  {translations[locale].titles.checkindex}
                </Title>
              </Group>
            </Box>
            <Box className="Box">
              <Group justify="center" gap="xl" mt="md">
                <Title order={3}>
                  {translations[locale].titles.checkindexLink}
                </Title>
              </Group>
              ...
              <Group justify="center" gap="xl" mt="md">
                ...
                <Select
                  placeholder={
                    translations[locale].placeholders.entity_checkindex
                  }
                  value={mode}
              ...
              <>
                ...
                <Grid>
                  <Grid.Col span={3}>
                    ...
                    >
                      <Text fw={700}>
                        Total checked links: {results.length}
                      </Text>
                      <Text fw={700}>
                        Indexed links:{" "}
                        {
                          results.filter(
                            (item) => item.indexed.toLowerCase() === "index"
                          ).length
                        }
                      </Text>
                      <Text fw={700}>
                        ...
                        {
                          results.filter(
                            (item) => item.indexed.toLowerCase() === "no index"
                          ).length
                        }
                      </Text>
                      <Text fw={700}>
        ...
        <Grid.Col span={4}>
          <Card
            py="sm"
            style={{ width: "50%", zIndex: 1, position: "relative" }}
            withBorder
          >
            <Card.Section>
              <Image
